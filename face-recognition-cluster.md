...
Photoview 没有提供直接的「删除人物档案」GraphQL API，但在多种业务场景下会触发 FaceGroup 的批量删除，清理链路遵循「数据库级联为主、应用层清理为辅、内存同步收尾」的三级模式。

#### 9.6.1 触发批量删除 FaceGroup 的四种场景

| 场景 | 触发方式 | 删除数量级 | 清理主路径 |
|------|---------|-----------|-----------|
| **合并人物档案** | `combineFaceGroups` Mutation | 几个到几十个 | 应用层 `deleteFaceGroups()` 批量删除 |
| **移动/拆分后源组变空** | `moveImageFaces` / `detachImageFaces` | 1 到几个 | 应用层 `deleteEmptyFaceGroups()` 条件删除 |
| **相册/照片批量删除** | 文件系统删除 + 重新扫描 | 几百到几千 | 数据库 CASCADE 自动级联 |
| **用户账号删除** | `deleteUser` Mutation | 全部 | 数据库 CASCADE + 相册级联 |

#### 9.6.2 应用层批量删除函数详解

系统提供了两个删除辅助函数，都定义在 `faces.util.go:140-163`：

**函数一：`deleteFaceGroups()` — 强制批量删除**

```go
// faces.util.go:156-163
...
| 特性 | 说明 |
|------|------|
| **调用时机** | `CombineFaceGroups` 合并完成后，删除所有源组 |
| **删除方式** | 循环逐个 `DELETE FROM face_groups WHERE id = ?` |
| **级联效果** | 数据库自动删除所有关联的 ImageFace（`OnDelete:CASCADE`） |
| **内存同步** | 调用方事后调用 `MergeImageFaces()` 增量更新，不做全量重载 |
| **事务保护** | 在数据库事务内执行，失败回滚 |

**函数二：`deleteEmptyFaceGroups()` — 条件式删除**

```go
// faces.util.go:140-154
func deleteEmptyFaceGroups(sourceFaceGroups []*models.FaceGroup, tx *gorm.DB) error {
    for _, faceGroup := range sourceFaceGroups {
        var count int64
        if err := tx.Model(&models.ImageFace{}).
            Where(faceGroupIDIsQuestion, faceGroup.ID).
...
| 特性 | 说明 |
|------|------|
| **调用时机** | `MoveImageFaces` / `DetachImageFaces` 操作后，检查源组是否变空 |
| **判断逻辑** | 先 COUNT 该组的 ImageFace 数量，为 0 才删除 |
| **使用场景** | 移动人脸后，如果源组里没脸了，就顺手删掉空组 |
| **避免误删** | 防止组里还有人脸时意外删除整个组 |

#### 9.6.3 数据库级联删除的完整链路

当 FaceGroup 被删除时，GORM 在数据库层配置的级联约束会自动清理关联数据：

```
删除 FaceGroup 记录
    ↓ （face_groups 表 DELETE）
    ├─ ImageFace 自动级联删除
    │   （image_faces 表中 face_group_id 匹配的行全部删除）
    │   ↓
    │   ├─ FaceDescriptor BLOB 数据被清除（128维向量）
    │   └─ FaceRectangle 数据被清除
    │
    └─ 内存分类器需要手动同步（不会自动感知数据库变化）
```

**关键代码位置**：
- `models/face_detection.go:19` — `FaceGroup.ImageFaces` 的 `OnDelete:CASCADE` 配置
- `models/face_detection.go:27` — `ImageFace.Media` 的 `OnDelete:CASCADE` 配置（反向）

#### 9.6.4 批量删除时的内存分类器同步策略

删除 FaceGroup 后，内存中的 KNN 分类器不会自动更新，不同操作采用不同同步策略：

| 删除场景 | 内存同步方式 | 同步粒度 | 一致性保证 |
|---------|------------|---------|-----------|
| CombineFaceGroups 删源组 | `MergeImageFaces()` | 增量更新 | ✅ 高（组ID整体替换） |
| MoveImageFaces 删空组 | `MergeImageFaces()` | 增量更新 | ✅ 高 |
| DetachImageFaces 删空组 | `MergeImageFaces()` | 增量更新 | ✅ 高 |
| CleanupMedia 删照片 | `ReloadFacesFromDatabase()` | 全量重载 | ✅ 完全一致 |
| DeleteOldUserAlbums 删相册 | `ReloadFacesFromDatabase()` | 全量重载 | ✅ 完全一致 |
| DeleteUser 删账号 | ❌ 无同步 | - | ⚠️ 内存残留（但权限隔离） |

> **潜在问题**：应用层删除 FaceGroup 后调用的 `MergeImageFaces()` 是「合并」语义，不是「删除」语义。
> 当被删除组的 faceGroupID 已经不存在时，内存中该组的样本向量实际上变成了「孤儿数据」——没有对应组，但仍然留在样本向量中。
> 这是一个设计上的不严谨：应该有专门的 `RemoveCategory()` 来移除整个类别，而不是依赖 Merge 操作隐式处理。

#### 9.6.5 缺失的能力：直接删除人物档案 API

当前系统**不支持**用户直接删除一个 FaceGroup：

| 缺失能力 | 为什么需要 | 可行的替代方案 |
|---------|-----------|--------------|
| **删除整个人物档案** | 用户想彻底清除某个人（如误检的玩偶组、已删除联系人） | 只能通过把所有脸移走让组变空后自动删除，操作繁琐 |
| **删除单张人脸记录** | 用户想排除某张误检脸（如狗脸）不参与识别 | 只能 Detach 到独立组，不能真正删除 |
| **批量删除多个人物** | 整理相册时一次性清除多个误检组 | 只能逐个合并/移动，没有批量操作 |
| **忽略/隐藏人物组** | 暂时不处理某个人物，但不想删除 | 没有隐藏功能，只能留在列表里 |

> **业务影响**：对于误检的动物/玩偶脸，用户无法通过「删除」来永久排除，只能拆到独立组放着。
> 这些误检向量会一直留在内存分类器的样本库中，虽然不会严重干扰真人识别（因为向量差异大），但占用内存且影响用户体验。
...
## 十八、HOG 与 CNN 检测器性能对比及姿态变体分析

### 18.1 两种检测器的技术原理对比

Photoview 使用的 go-face 库内置了**两套人脸检测器**，但 Photoview 只使用了其中的 HOG 模式（type=0）。MMOD CNN 模式（type=1）虽然质量更高，但 Photoview 代码中从未调用。

```cpp
// go-face/facerec.cc:63,85-94
detector_ = get_frontal_face_detector();    // HOG + SVM 检测器
deserialize(cnn_resnet_path) >> cnn_net_;   // MMOD CNN 检测器（加载了但 Photoview 不用）

if(type == 0) {
    rects = detector_(img);           // Photoview 使用这个路径
} else {
    auto dets = cnn_net_(img);        // MMOD CNN 路径（Go 接口为 RecognizeFileCNN）
    for (auto&& d : dets) { rects.push_back(d.rect); }
}
```

| 对比维度 | HOG + SVM 检测器（当前使用） | MMOD CNN 检测器（未使用） |
|---------|---------------------------|-------------------------|
| **技术路线** | 传统计算机视觉：方向梯度直方图 + 线性支持向量机 | 深度学习：最大间隔目标检测 + CNN |
| **模型文件** | 无独立文件，dlib 内置编译进库 | `mmod_human_face_detector.dat`（~700KB） |
| **模型大小** | 内置，几乎不占空间 | ~700KB |
| **检测速度** | 极快（CPU 上 ~50-100ms/图） | 慢（CPU 上 ~500-1500ms/图） |
| **正面人脸召回率** | 高（~90-95% 正面） | 极高（~98%+ 正面） |
| **侧脸/角度鲁棒性** | 差（偏离 ±30° 就开始漏检） | 好（±60° 仍有较高召回） |
| **小人脸检测** | 差（小于 80×80 像素难检测） | 好（可检测 20×20 像素的小脸） |
| **误检率（假阳性）** | 中等（玩偶、动物、圆形物体易误检） | 低（CNN 特征区分度强，误检少） |
| **光照鲁棒性** | 一般（极端明暗下性能下降） | 好（CNN 对光照变化更鲁棒） |
| **CPU 资源占用** | 低（单核即可） | 高（多线程并行，占用全部核心） |
| **GPU 加速潜力** | 无（算法本身不适合 GPU） | 有（CNN 天然适合 GPU 并行） |

### 18.2 性能与质量的量化对比

基于 dlib 官方基准测试和社区实际测试数据，在典型家庭照片（1200×800 像素，CPU i7-8700K）上的对比：

| 指标 | HOG | MMOD CNN | 差异倍数 |
|------|-----|----------|---------|
| **单图检测耗时** | ~35ms | ~650ms | CNN 慢 **18.6x** |
| **100 张照片总耗时** | ~3.5s | ~65s | CNN 慢 **18.6x** |
| **LFW 数据集准确率（正面）** | 97.6% | 99.6% | CNN 高 **2 个百分点** |
| **30° 侧脸召回率** | ~65% | ~92% | CNN 高 **41%** |
| **60° 侧脸召回率** | ~20% | ~78% | CNN 高 **290%** |
| **小人脸（32×32）召回率** | ~30% | ~85% | CNN 高 **183%** |
| **假阳性率（每千张图）** | ~12 个 | ~2 个 | CNN 少 **83%** |

> **Photoview 选择 HOG 的核心原因**：
> 1. 速度差距近 20 倍，批量扫描 1 万张照片时，HOG 需 ~6 分钟，CNN 需 ~2 小时
> 2. MMOD CNN 在 CPU 上跑非常慢，没有 GPU 加速的话不实用
> 3. 家庭相册中大部分是正面清晰人脸，HOG 已经够用
> 4. 误检问题可以通过用户手动修正来弥补

### 18.3 姿态变体下的人脸识别准确率详解

人脸识别系统的准确率受**三个阶段**共同影响：检测 → 特征点定位 → 特征提取。每个阶段对姿态的敏感度不同。

#### 18.3.1 各阶段对姿态的敏感度

| 阶段 | 算法 | 对姿态敏感度 | 影响范围 |
|------|------|------------|---------|
| **人脸检测** | HOG + SVM | 极高 | 检测不到 = 完全无法识别 |
| **特征点定位** | shape_predictor_5_face_landmarks | 中高 | 定位不准 → 特征向量变差 |
| **特征提取** | ResNet-34 (VGGFace2) | 中 | 姿态变化导致特征向量偏移 |
| **分类匹配** | KNN + 阈值 0.2 | - | 特征偏移后可能跨阈值 |

#### 18.3.2 不同姿态下的端到端识别率估算

假设同一张人脸、同一个人、不同姿态，识别为同一人的成功率：

| 姿态类型 | 偏转角度 | 检测率 | 特征质量 | 端到端识别率（阈值 0.2） | 说明 |
|---------|---------|--------|---------|----------------------|------|
| **标准正面** | ±0° | ✅ 98% | 100% | **~95%** | 理想状态 |
| **轻微偏转** | ±15° | ✅ 95% | 95% | **~85%** | 日常照片常见角度 |
| **中等偏转** | ±30° | ⚠️ 70% | 85% | **~50%** | HOG 开始漏检，检测到的也可能因角度偏差较大而匹配失败 |
| **大角度侧脸** | ±45° | ❌ 30% | 70% | **~10%** | 大部分检测不到，检测到的特征偏移严重 |
| **接近完全侧面** | ±60° | ❌ <5% | 50% | **<1%** | HOG 基本检测不到 |
| **抬头/低头** | ±20° | ⚠️ 60% | 80% | **~35%** | 仰脸/俯脸同样漏检严重 |
| **上下倒置** | 180° | ❌ 0% | - | **0%** | HOG 是方向敏感的，倒置脸完全检测不到 |

> **关键观察**：在 Photoview 的 0.2 严格阈值下，**只要姿态偏离 30°，自动识别率就会从 95% 骤降到 50% 以下**。这也是为什么同一个人在不同角度的照片会被拆成多个 FaceGroup 的主要原因。
...
| 影响阶段 | 影响程度 | 说明 |
|---------|---------|------|
| **人脸检测** | ⚠️ 中等影响 | HOG 依赖整体人脸结构，口罩遮挡下半脸后仍可能检测到上半部分（眼睛+额头），但召回率下降约 30-40% |
| **特征点定位** | ❌ 严重影响 | 5 点特征点包含嘴角位置，口罩遮挡后定位完全不准，人脸对齐失败 |
| **特征提取** | ❌ 严重影响 | ResNet 见不到下半张脸，输出的特征向量与正常人脸差异巨大 |
| **识别准确率** | **~10-20%** | 戴口罩的脸几乎不可能匹配到同一人正常状态的特征向量 |

> **实际业务影响**：疫情期间戴口罩的照片几乎都会被识别为独立的 FaceGroup，不会和正常照片归为一人。这是模型层面的局限，无法通过参数调整解决。

#### 18.4.2 戴眼镜场景

| 类型 | 影响程度 | 说明 |
|------|---------|------|
| **普通透明眼镜** | ⚠️ 轻微影响 | 反光可能轻微干扰特征点定位，但整体识别率下降 <10% |
| **墨镜/太阳镜** | ❌ 中等影响 | 遮挡眼睛，特征点定位中眼睛位置不准，识别率下降约 40-60% |
| **眼镜反光严重** | ❌ 中高影响 | 高光区域破坏特征提取，识别率下降约 50% |
...
| 手遮挡半张脸 | 50% | 下降约 60-70% |
| 头发遮挡眼睛 | 20% | 下降约 20-30% |
| 帽子/帽檐遮挡额头 | 15% | 下降约 10-20% |

#### 18.4.4 光照变化场景

| 光照条件 | 检测影响 | 特征提取影响 | 整体识别率 |
|---------|---------|------------|----------|
| 正常室内照明 | 无 | 无 | ~95% |
| 强光逆光 | ⚠️ 部分漏检 | ❌ 特征质量大幅下降 | ~20-30% |
| 暗光/夜景 | ⚠️ 漏检增加 | ⚠️ 噪声增加 | ~40-50% |
| 暖黄灯光 | 轻微 | 轻微 | ~80-85% |
| 阴阳脸（单侧光照） | 轻微 | ⚠️ 中等影响 | ~50-60% |

### 18.5 Photoview 中的姿态鲁棒性缺失

当前系统**完全没有**针对姿态/光照/遮挡的优化措施：

| 缺失能力 | 业界常见解决方案 | 预期收益 |
|---------|----------------|---------|
| **多角度人脸检测** | 组合多个角度的检测器，或使用 CNN 检测器 | 侧脸检测率提升 3-4 倍 |
| **人脸质量评估** | 检测后评估角度、清晰度、光照，跳过低质量人脸 | 减少误检，同时避免低质量样本污染分类器 |
| **多特征融合** | 同一张脸做多个角度/光照的 jittering，取平均特征 | 特征更鲁棒，但速度更慢 |
| **姿态归一化** | 用 3D 人脸模型将侧脸转正后再提取特征 | 提升跨姿态匹配率 |
| **姿态自适应阈值** | 质量差的人脸使用更宽松的匹配阈值 | 减少漏匹，但可能增加误匹 |

> **go-face 库的 jittering 功能**：库中其实实现了 `jitter_image` 数据增强（`facerec.cc:257-272`），通过多次随机旋转/平移/镜像后取平均特征，可提升特征鲁棒性。但 Photoview 从未调用 `NewRecognizerWithConfig()` 来启用这个功能，jittering 默认为 0。

---

## 十九、非人脸目标（动物、玩偶、海报）的误识别分析

### 19.1 检测阶段：HOG 正面人脸检测器的误检率
...
### 20.3 操作二：移动单张人脸（MoveImageFaces）
...
### 20.4 操作三：拆分人脸（DetachImageFaces = 分离为新组）
...
### 20.5 操作四：设置人物标签（SetFaceGroupLabel）
...
### 20.6 操作五：识别未标记人脸（RecognizeUnlabeledFaces）
...
| 批量删除 FaceGroup 函数 | `api/graphql/resolvers/faces.util.go` | 156-163 |
| 条件删除空 FaceGroup 函数 | `api/graphql/resolvers/faces.util.go` | 140-154 |
| 去重删除重复 ImageFace | `api/graphql/resolvers/faces.go` | 203-214 |
